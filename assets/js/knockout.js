...
ko.LicenseMap = ko.observableArray([]);
//< data-bind="check_LicenseAction: 'action1', action: 'xxx'"  />
ko.bindingHandlers.check_LicenseAction = {
	    update: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        // This will be called when the binding is first applied to an element
	        // Set up any initial state, event handlers, etc. here
	    	var self = $(element);
	        ...
	        var flag = false;
	        if(ko.LicenseMap()){
	        	if(ko.LicenseMap().indexOf(value)!=-1){
	        		if(allBindings()["action"]){
		        		if(ko.actionsMap()){
...
ko.bindingHandlers.ie9download = {
	    init: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
	    	if(!(navigator.userAgent.indexOf("MSIE 9.0") > -1)) return;
	    	element.onclick=function(){
	    		var _href = element.href;
	    		if(_href.indexOf("&downloadtime") > -1){
	    			_href = _href.substring(0,_href.indexOf("&downloadtime"));
	    		}
	    		if(_href.indexOf("?")>-1){
	    			element.href=_href+"&downloadtime="+new Date().getTime();
	    		}else{
	    			element.href=_href+"?downloadtime="+new Date().getTime();
	    		}
