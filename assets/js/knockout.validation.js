...
kv.rules['positiveIntegerRuleByArray'] = {//正整数校验(分号分隔)
		  validator: function (val, params) {
			  	var flag = true; 
			  	if(val) {
			  		var vals = val.split(";");
				  	for(var i in vals) {
				  		if(vals[i] && !(/^[1-9]\d*$/.test(vals[i]))) {
				  			flag = false;
				  			break;
				  		}
				  	}
...
kv.rules['ip'] = {
		  validator: function (val, params) {//params为true时支持cidr校验
			    ...
			    if(cidr){
                	this.message = 'IP格式不正确,正确格式如:192.168.1.0/24 或 192.168.1.1'
                }
                ...
                if(!val){
	  	        ...
	  	        if( /^([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/.test(val)){
	  	        	return true;
	  	        }else if(cidr &&  /^([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\/([0-2][0-9]{0,1}|3[0-2])$/.test(val)){
	  	        	return true;
...
kv.rules['ipcidr'] = {
		  validator: function (val, params) {//params为true时支持cidr校验 
			    ...
			    if(cidr){
              	this.message = 'IP格式不正确,正确格式如:192.168.1.0/24'
              }

              if(!val){
              	return true;
              }
	  	       if(cidr &&  /^([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\/([0-2][0-9]{0,1}|3[0-2])$/.test(val)){
	  	        	return true;
	  	        }else if(params &&  params.supportSpecial){//特殊处理
	  	        	return val == "0.0.0.0/0";
	  	        }
